...
var scores = new Array(); 
	...
	for (var i = 0; i < elements.length; i++) 
	{
		...
		if(turn == 'X')
		{
			var hover = elments[i].innerHTML='X';
			hover.style.color ="#D3D3D3";
		}
		
		else if(turn == 'O')
		{
			var hover = elments[i].innerHTML='O';
			hover.style.color ="#D3D3D3";
		}
...
console.log(win_pattern.length);
		...
		if(player1 == "" || player2 == "")
		{
			...
			for (var row = 1; row <= game_type; row++){
				for (var col = 1; col <= game_type; col++) {
					...
					button.setAttribute("onclick", "markCheck(this),Timer()");
